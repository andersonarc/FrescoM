...
class Initialization(Frame):
    ...
    def init_ui(self):
        ...
        remember_bottom_left_position_button = tk.Button(self,
                                                         text='Remember bottom left position',
                                                         command=self.remember_bottom_left_position)
        remember_bottom_left_position_button.grid(column=0, row=3, ipadx=2, pady=2, sticky=tk.W)

        remember_top_right_position_button = tk.Button(self,
                                                       text='Remember top right position',
                                                       command=self.remember_top_right_position)
        remember_top_right_position_button.grid(column=0, row=4, ipadx=2, pady=2, sticky=tk.W)

        sync_plate_coordinates_button = tk.Button(self,
                                                  text="Sync plate coordinates",
                                                  command=self.sync_plate_coordinates)
        sync_plate_coordinates_button.grid(column=0, row=5, ipadx=2, pady=2, sticky=tk.W)

        self.plate_coordinates_label = Label(self, text='(0, 0) , (0, 0)')
        self.plate_coordinates_label.grid(column=0, row=6, ipadx=2, pady=2, sticky=tk.W)
    ...
    def remember_bottom_left_position(self):
        _thread.start_new_thread(self.fresco_xyz.remember_bottom_left_position, ())

    def remember_top_right_position(self):
        _thread.start_new_thread(self.fresco_xyz.remember_top_right_position, ())
